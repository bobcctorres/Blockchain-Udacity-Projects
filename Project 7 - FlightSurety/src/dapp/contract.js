...
export default class Contract {
    ...
    fundAirline(callback) {
        let self = this;
        const amountToSend = this.web3.utils.toWei("10", "ether");
        self.flightSuretyApp.methods
        .fundAirline()
    ...
    registerFlight(flight, callback) {
        ...
        self.flightSuretyApp.methods     
        .registerFlight(this.web3.utils.fromAscii(flight),time,this.airlines[0]).send({ from: self.owner}, callback);    
     }

    isFlightRegistered(flight,callback) {
        let self = this;
        self.flightSuretyApp.methods.isFlightRegistered(this.web3.utils.fromAscii(flight)).call({ from: self.owner}, callback);
     }
