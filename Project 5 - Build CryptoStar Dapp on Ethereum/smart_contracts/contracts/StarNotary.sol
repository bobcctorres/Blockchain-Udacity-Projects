...
contract StarNotary is ERC721 { 
    ...
    mapping(bytes32 => bool) public starHashIdMap;
    ...
    function createStar(string _name,  string _dec, string _mag, string _cent, string _story, uint256 _tokenId) public { 
        ...
        require(!checkIfStarExist(_cent, _dec, _mag));

        Star memory newStar = Star(_name, _story, _cent, _dec, _mag);
        tokenIdToStarInfo[_tokenId] = newStar;

        bytes32 hash = generateStarHash(_cent, _dec, _mag);
        starHashIdMap[hash] = true;
    ...
    function checkIfStarExist(string _cent, string _dec, string _mag) public view returns(bool) {
        return starHashIdMap[generateStarHash(_cent, _dec, _mag)];
    }

    function tokenIdToStarInfo(uint256 _tokenId) public view returns(string, string, string, string, string){
        return (tokenIdToStarInfo[_tokenId].name, tokenIdToStarInfo[_tokenId].story, tokenIdToStarInfo[_tokenId].cent, tokenIdToStarInfo[_tokenId].dec, tokenIdToStarInfo[_tokenId].mag);
    }

    function generateStarHash(string _cent, string _dec, string _mag) private pure returns(bytes32) {
